...
#' y_var is ordinal.
#' @param clr_var Optional variable (vector) used to colour bubbles. Up to five 
#' levels of the vector may be plotted as different bubbles. If omitted, a
#' heat map is plotted without bubbles, with grids populated by their value of 
#' n (the number of rows/studies) and coloured accordingly across a gradient.
...
#' data <- subset(data, loc == "USA/Canada")
#' attach(data)
...
bubbleplot <- function(x_var, 
  ...
  #create static plot
  if(length(unique(clr_var)) <= 1){
    x <- data %>%
      ggplot2::ggplot(mapping = ggplot2::aes (x=x_var2, #offset variables according to their matrix location
                                              y=y_var2,
                                              label1 = x_var,
                                              label2 = y_var,
                                              label3 = clr_var,
      ...
      ggplot2::scale_fill_gradient(low = '#A5CAD2', high = '#FF7B89',
                                 name = 'Number of\nstudies\n') +
